...
### 【追補3】「発車直後なのにバスがまだ来ていない」不具合の完全是正

ユーザー提供画像（古泉・上大岡方面: 11:48発なのに「発車直後」と表示され、直下には「あと2駅手前」と表示される矛盾）の分析と是正内容：

1. **実効遅延・推定到着時刻の動的自動補正 (`timetable-service.js`)**
   - **問題**: ODPT API の `odpt:delay` が 0（未配信または集計ラグ）の場合、バスが手前2駅にいるにもかかわらず所定時刻（11:48）がそのまま予定時刻として使われていた。
   - **是正**: バスが手前（`at_stop`, `approaching`, `en_route`）にいる場合、手前停留所数（1駅約1.5〜2分）から推定到着時刻を算出。直近便（±35分以内）で推定到着時刻が所定時刻を過ぎている場合は、現在地から割り出した実効遅延（例: +3〜4分）を自動適用し、`actualDepartureTime` を `11:51` 等に延伸。右上の遅延バッジも「定刻」から「+○分遅れ」に正しく同期させ、カウントダウンも正の秒数を維持。

2. **手前バス存在時の「発車直後」完全禁止 (`timetable-service.js`, `ui-helpers.js`)**
   - **問題**: `formatCountdown` が時刻差分のみで判定し、`totalSec < 0`（所定時刻超過）になると無条件に「発車直後」を返していた。
   - **是正**: 第3引数に `locationStatus` を受け取り、手前バスがいる間は `totalSec <= 0` であっても「発車直後」を完全に抑止。
     - のりば停車中（`at_stop`）: **「停車中」**（乗車可能）
     - 1駅手前（`approaching` / `stopsAway: 1`）: **「まもなく到着」**
     - 手前走行中（`en_route`）: **「遅延 接近中 (あと○駅)」**
     - 実際に通過（`passed`）した場合、または位置情報のない便のみ「発車直後」を表示。

3. **DOM属性と毎秒タイマーの完全同期 (`render-stop-view.js`, `app.js`)**
   - **問題**: 画面描画後に1秒タイマー（`updateCountdownElements`）が走り、ダイヤ時刻との差分から「発車直後」へ強制上書きしていた。
   - **是正**: 先発便カードに `data-status` と `data-stops-away` を埋め込み、1秒タイマー側でも `locationStatus` を考慮したカウントダウン更新を実施。また手前バスがいる間は -120秒を超えても強制全画面リフレッシュを行わないようガード。

---

### 【追補4】遅延見積もりアルゴリズムの適正化・過大見積もり解消（チューニング）

「遅延規模を大きく見積もりすぎではないか」というフィードバックに基づき、遅延算出アルゴリズムを現実の実走データに即して最適化：

1. **手前所要時間換算レートの適正化 (`timetable-service.js`)**
   - **問題**: 1駅あたり「2分」固定で計算していたため、2駅手前で一気に「+4分遅れ」、3駅手前で「+6分遅れ」と過大に跳ね上がっていた。
   - **是正**: 都市部バス停の短区間（200〜400m）の実走値に合わせ、**1駅あたり約1.0〜1.1分**（2駅手前: 約2分、3駅手前: 約3分）に短縮チューニング。体感と一致する自然な到着見込み時間を実現。

2. **公式遅延（`odpt:delay`）の最優先化**
   - **問題**: 横浜市交通局がリアルタイムGPSで「1分遅れ」と公式配信していても、アプリ側の手前駅数推定（4分）が上書きしてしまっていた。
   - **是正**: 公式遅延が存在し、現在時刻が遅延見込み時刻の前である場合は、交通局公式の遅延を最優先で維持。所定時刻を超過しても公式遅延が未更新（0秒）のままの場合のみ、手前駅数からの差分を適用する二段階判定に刷新。
